...
public class Player {
    ...
    private void initializeAnimations() {
        try {
            ...
            Gdx.app.error("Player", "Error loading textures: " + e.getMessage());
            createPlaceholderTextures();
        }
    }

    private void createPlaceholderTextures() {
        Pixmap pixmap = new Pixmap(36, 36, Pixmap.Format.RGB888);
        pixmap.setColor(com.badlogic.gdx.graphics.Color.GREEN);
        pixmap.fill();
        Texture placeholder = new Texture(pixmap);
        pixmap.dispose();

        TextureRegion[] placeholderFrames = new TextureRegion[4];
        for (int i = 0; i < 4; i++) {
            placeholderFrames[i] = new TextureRegion(placeholder);
        }

        runAnimation = new Animation<>(1f/12f, placeholderFrames);
        runAnimation.setPlayMode(Animation.PlayMode.LOOP);
        flyTexture = new TextureRegion(placeholder);
        currentFrame = flyTexture;
    }
